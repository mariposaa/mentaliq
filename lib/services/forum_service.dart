...
class ForumService {
  ...
  static String get contentLanguageName {
    const names = {'tr': 'Türkçe', 'en': 'English', 'de': 'Deutsch', 'es': 'Español', 'ar': 'العربية'};
  ...
  static Stream<List<ForumPost>> watchFeedPosts() {
    return AuthService.firestore
        ...
        .limit(100)
        .snapshots()
        .map((s) => s.docs.map((d) => ForumPost.fromFirestore(d)).where((p) => p.dailyQuestionId == null).toList());
  }

  static Future<ForumPost?> getPost(String postId) async {
    final doc = await AuthService.firestore.collection('forum_posts').doc(postId).get();
  ...
  static Future<void> updatePost(String postId, {String? text, String? imageUrl}) async {
    final uid = AuthService.userId!;
  ...
  static Future<String> _uploadPostImageBytes(String uid, Uint8List bytes) async {
...
${post.text}''';
      ...
      final translated = await GeminiService.generateResponse(prompt, 'translation');
      
      // Temizle (Gemini bazen ekstra metin ekleyebilir)
      final cleanedTranslation = translated.trim();
      
      // Firestore'a cache'le
      await AuthService.firestore.collection('forum_posts').doc(post.id).update({
  ...
  static Stream<List<ForumComment>> watchComments(String postId) {
    return AuthService.firestore
        ...
        .snapshots()
        .map((s) => s.docs.map((d) => ForumComment.fromFirestore(d, postId: postId)).toList());
  }
  ...
  static Future<void> updateComment(String postId, String commentId, String text) async {
    final uid = AuthService.userId!;
  ...
  static Future<void> _notifyPostAuthor(String postId, {required String type, String? commentId}) async {
    try {
      final postDoc = await AuthService.firestore.collection('forum_posts').doc(postId).get();
  ...
  static Future<List<ForumDailyQuestion>> getDailyQuestionsForDate(DateTime date) async {
  ...
  static Future<void> setDailyQuestion({
    ...
    if (id != null && id.isNotEmpty) {
      await AuthService.firestore.collection('forum_daily_questions').doc(id).update({
        'text': text,
        'language': language,
        'date': Timestamp.fromDate(start),
      });
  ...
  static Future<void> deleteDailyQuestion(String id) async {
    await AuthService.firestore.collection('forum_daily_questions').doc(id).delete();
  }
}
