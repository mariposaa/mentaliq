...
class StyleService {
  ...
  static String _normalizeCategoryKey(String value) {
    ...
    if (v == 'top' || v.contains('üst') || v.contains('ust')) return 'top';
    if (v == 'bottom' ||
        v.contains('alt') ||
        v.contains('pantolon') ||
        v.contains('etek')) {
    ...
    if (v == 'shoes' || v.contains('ayakk')) return 'shoes';
    if (v == 'accessory' || v.contains('aksesuar')) return 'accessory';
    return 'other';
  }

  static String _normalizeSeasonKey(String value) {
    final v = value.trim().toLowerCase();
    if (v == 'summer' || v.contains('yaz')) return 'summer';
    if (v == 'winter' || v.contains('kış') || v.contains('kis'))
      return 'winter';
    if (v == 'spring' || v.contains('bahar')) return 'spring';
    if (v == 'autumn' ||
        v.contains('sonbahar') ||
        v.contains('guz') ||
        v.contains('güz')) {
  ...
  static Future<StyleItem?> addToCloset(XFile imageFile) async {
    ...
    try {
      ...
      try {
        if (kIsWeb) {
          debugPrint(
              'StyleService: Uploading Data (Web) - Timeout set to 30s...');
          final task = ref.putData(
              imageBytes, SettableMetadata(contentType: 'image/jpeg'));
          await task
              .whenComplete(() => debugPrint('StyleService: putData complete'))
        ...
        throw Exception(
            'Resim yükleme zaman aşımına uğradı. Lütfen internet bağlantını ve Firebase Storage kurallarını kontrol et.');
      }
      ...
      final newItem = StyleItem(
        id: '',
        imageUrl: imageUrl,
        category: analysisResult['categoryKey'] ?? 'other',
        tags: List<String>.from(analysisResult['tags'] ?? []),
        season: analysisResult['seasonKey'] ?? 'all_season',
  ...
  /// Remove an item from closet and cleanup image storage
  static Future<bool> deleteClosetItem(StyleItem item) async {
    final userId = AuthService.userId;
    ...
    try {
      ...
      // Best-effort cleanup for uploaded image.
      try {
        final ref = _storage.refFromURL(item.imageUrl);
        await ref.delete();
      } catch (e) {
        debugPrint('StyleService: storage cleanup skipped: $e');
      }

      return true;
    } catch (e) {
      debugPrint('StyleService delete error: $e');
      return false;
    }
  }
  ...
  static Future<String> getClosetAsText() async {
    ...
    for (var i = 0; i < snapshot.docs.length; i++) {
      ...
      buffer.writeln(
          '${i + 1}. ${item.category}: ${item.tags.join(', ')} (${item.season})');
    }
    return buffer.toString();
  }

  /// Gemini ile tek seferlik görsel analizi
  static Future<Map<String, dynamic>> _analyzeStyleItem(
      Uint8List imageBytes) async {
    const prompt = '''
Bu kıyafeti bir moda uzmanı gibi analiz et ve SADECE aşağıdaki JSON formatında yanıt ver:
{
  "category_key": "top/bottom/outerwear/shoes/accessory/other",
...
''';
    ...
    try {
      final response = await GeminiService.analyzeImage(imageBytes, prompt,
          category: 'stil_danismanligi');
  ...
  static Map<String, dynamic> _parsingSimulatedJson(String text) {
    try {
      ...
      return {
        'categoryKey': _normalizeCategoryKey(
          (data['category_key'] ?? data['category'] ?? 'other').toString(),
        ),
        'tags': List<String>.from(data['tags'] ?? ['general']),
        'seasonKey': _normalizeSeasonKey(
          (data['season_key'] ?? data['season'] ?? 'all_season').toString(),
        ),
